  ...
  const handleTopup = async () => {
    ...
    try {
      ...
      const options = {
        ...
        name: "SecureKYC Platform Wallet",
        description: "Top up platform wallet",
        ...
        handler: async (response: Record<string, string>) => {
          ...
          else { setPaymentResult({ success: false, message: verifyData.error || "Payment verification failed" }); }
          setProcessing(false);
        },
      ...
      razorpay.open();
    } catch (e) { console.error("Top-up error:", e); setPaymentResult({ success: false, message: "Top-up failed. Please try again." }); setProcessing(false); }
  };
    ...
    <div className="min-h-screen flex bg-surface">
      <aside className="hidden lg:flex w-60 flex-col bg-surface-container-lowest border-r border-outline-variant/20 fixed top-0 left-0 h-full z-10">
        ...
        <nav className="flex-1 p-3 space-y-1">
          ...
          <Link href="/admin/wallet" className="nav-item active">
      ...
      <main className="flex-1 lg:ml-60 min-h-screen">
        <header className="sticky top-0 z-20 glass-nav border-b border-outline-variant/20 flex items-center justify-between px-6 h-14">
          <div>
            <h1 className="font-bold text-base text-on-surface">Platform Wallet</h1>
            <p className="text-[11px] text-on-surface-variant">Manage funds for KYC verifications</p>
          </div>
            ...
            <>
              <div className="bg-gradient-to-br from-primary to-primary-container rounded-2xl p-6 shadow-lg">
                <div className="flex items-center justify-between mb-4">
                  ...
                  {wallet?.lowBalance && (
                    <span className="flex items-center gap-1 text-xs font-semibold bg-red-500/20 text-red-100 px-2.5 py-1 rounded-full">
                      <span className="material-symbols-outlined text-[14px]">warning</span> Low Balance
                    </span>
                  )}
                </div>
                <p className="text-4xl font-bold text-on-primary mb-4">
                  {"\u20B9"}{wallet?.balanceFormatted?.toLocaleString("en-IN") || "0"}
                </p>
                <div className="grid grid-cols-3 gap-4">
                  {[
                    { label: "Free PAN", value: wallet?.freeCredits?.pan || 0, icon: "credit_card" },
                    { label: "Free Aadhaar", value: wallet?.freeCredits?.aadhaar || 0, icon: "fingerprint" },
                    { label: "Free Credit Score", value: wallet?.freeCredits?.creditScore || 0, icon: "analytics" },
                  ].map((item) => (
                    <div key={item.label} className="bg-white/10 rounded-xl p-3">
                      <span className="material-symbols-outlined text-on-primary/60 text-[16px]">{item.icon}</span>
                      <p className="text-lg font-bold text-on-primary mt-1">{item.value}</p>
                      <p className="text-[10px] text-on-primary/70">{item.label}</p>
                    </div>
                  ))}
                </div>
              </div>
              <div className="bg-surface-container-lowest rounded-2xl border border-outline-variant/20 shadow-card p-6">
                <h2 className="font-bold text-sm text-on-surface mb-4">Top Up Wallet</h2>
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
                  {PRESET_AMOUNTS.map((amount) => (
                    <button key={amount} onClick={() => { setTopupAmount(amount); setIsCustom(false); }}
                      className={`p-3 rounded-xl border-2 text-center transition-all ${!isCustom && topupAmount === amount ? "border-primary bg-lavender shadow-primary-glow" : "border-outline-variant/30 hover:border-primary/50"}`}>
                      <p className="text-lg font-bold text-on-surface">{"\u20B9"}{(amount / 100).toLocaleString("en-IN")}</p>
                    </button>
                  ))}
                </div>
                <div className="flex gap-3">
                  <div className="flex-1">
                    <input type="number" placeholder="Custom amount (min 500)"
                      className="w-full h-12 px-4 border border-outline-variant rounded-xl text-sm bg-surface-container focus:outline-none focus:border-primary"
                      value={customAmount} onChange={(e) => { setCustomAmount(e.target.value); setIsCustom(true); }} onFocus={() => setIsCustom(true)} />
                  </div>
                  <button onClick={handleTopup} disabled={processing}
                    className="h-12 px-6 bg-primary text-on-primary rounded-full text-sm font-semibold hover:opacity-90 active:scale-95 transition-all shadow-primary-glow disabled:opacity-50 flex items-center gap-2">
                    {processing ? (
              ...
              {paymentResult && (
                <div className={`flex items-center gap-3 p-4 rounded-xl border ${paymentResult.success ? "bg-green-50 border-green-200 text-green-700" : "bg-red-50 border-red-200 text-red-700"}`}>
                  <span className="material-symbols-outlined text-[20px]" style={{fontVariationSettings:"'FILL' 1"}}>{paymentResult.success ? "check_circle" : "error"}</span>
                  <p className="text-sm font-medium flex-1">{paymentResult.message}</p>
                  <button onClick={() => setPaymentResult(null)} className="text-[14px]">X</button>
                </div>
              )}
              <div className="bg-surface-container-lowest rounded-2xl border border-outline-variant/20 shadow-card overflow-hidden">
                <div className="p-4 border-b border-outline-variant/20">
                  <h2 className="font-bold text-sm text-on-surface">Transaction History</h2>
                </div>
                {transactions.length === 0 ? (
                  <div className="flex flex-col items-center justify-center py-12 text-on-surface-variant">
                    <span className="material-symbols-outlined text-[40px] mb-2 opacity-40">receipt_long</span>
                    <p className="text-sm font-medium">No transactions yet</p>
                    <p className="text-xs opacity-70">Top up your wallet to start</p>
                  </div>
                ) : (
                    ...
                    {transactions.map((tx) => {
                      ...
                      return (
                        <div key={tx.id} className="flex items-center gap-3 px-4 py-3 hover:bg-surface-container/30 transition-colors">
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="text-sm font-medium text-on-surface truncate">{tx.description || tx.type}</p>
                            <p className="text-[11px] text-on-surface-variant">{new Date(tx.createdAt).toLocaleString("en-IN")}</p>
                          </div>
                          <div className="text-right">
                            <p className={`text-sm font-bold ${tx.amount >= 0 ? "text-green-600" : "text-red-600"}`}>
                              {tx.amount >= 0 ? "+" : "-"}{"\u20B9"}{Math.abs(tx.amountFormatted).toLocaleString("en-IN")}
                            </p>
                            <p className="text-[11px] text-on-surface-variant">Bal: {"\u20B9"}{tx.balanceAfterFormatted.toLocaleString("en-IN")}</p>
                          </div>
                        </div>
                      );
                    })}
